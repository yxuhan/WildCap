...
def make_model(config):
    ...
    if config['type'] == 'image_v1':
        model = models.ImageDenoiserModelV1(
            ...
            unet_cond_dim=config_cond['cond_channels'],
        ...
        model = models.ImageTransformerDenoiserModelV1(
            ...
            d_ff=config['d_ff'],
            in_features=config['input_channels']+config_cond['cond_channels'],
        ...
        model = models.ImageTransformerDenoiserModelV2(
            levels=levels,
            mapping=mapping,
            in_channels=config['input_channels']+config_cond['cond_channels'],
...
def make_denoiser_wrapper(config):
    ...
    data_channel = config['input_channels']
    if loss_config == 'karras':
        weighting = config.get('loss_weighting', 'karras')
        scales = config.get('loss_scales', 1)
        if not has_variance:
            return partial(
                layers.Denoiser, data_channel=data_channel, sigma_data=sigma_data, weighting=weighting, scales=scales
            )
