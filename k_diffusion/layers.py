...
class Denoiser(nn.Module):
    ...
    def __init__(self, inner_model, data_channel, sigma_data=1., weighting='karras', scales=1):
        super().__init__()
    ...
    def loss(self, input, noise, sigma, **kwargs):
        ...
        noised_input = torch.cat([
            noised_input[:, :self.data_channel], input[:, self.data_channel:]
        ], dim=1)
        model_output = self.inner_model(noised_input * c_in, sigma, **kwargs)
        target = (input - c_skip * noised_input) / c_out
        target = target[:, :self.data_channel]
        if self.scales == 1:
    ...
    def forward(self, input, sigma, **kwargs):
        c_skip, c_out, c_in = [utils.append_dims(x, input.ndim) for x in self.get_scalings(sigma)]
        return self.inner_model(input * c_in, sigma, **kwargs) * c_out + input[:, :self.data_channel] * c_skip
